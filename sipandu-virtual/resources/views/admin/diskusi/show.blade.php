...
<div class="mx-auto max-w-7xl space-y-7">
    ...
    <section class="border-b border-base-300 pb-6">
        <a
            ...
            class="btn btn-ghost btn-sm -ml-2 mb-4 rounded-xl text-base-content/70 hover:text-primary"
        >
        ...
        <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
                ...
                <h1 class="font-display mt-2 text-3xl font-semibold leading-tight text-base-content md:text-4xl">
                    {{ $periode->tahunAjaran->label ?? 'Tahun Ajaran' }}
                </h1>
    ...
    <section class="rounded-2xl border border-base-300 bg-base-200 p-4 sm:p-5">
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div class="flex items-start gap-3">
                ...
                <div>
                    ...
                    <p class="mt-1 text-sm leading-6 text-base-content/70">
                        Balas pertanyaan yang memerlukan arahan. Kunci thread jika pembahasan telah selesai agar percakapan tidak berubah lagi.
                    </p>
                </div>
            </div>
            ...
            <div class="rounded-xl border border-base-300 bg-base-100 px-4 py-3 text-sm">
                ...
                <p class="mt-1 flex items-center gap-2 font-semibold text-base-content">
        ...
        <div class="space-y-5">
            @forelse($threads as $thread)
                @php
                    ...
                    $inisialGuru = strtoupper(substr($thread->guru->nama_lengkap ?? 'G', 0, 1));
                @endphp
                ...
                <article class="overflow-hidden rounded-2xl border border-base-300 bg-base-100 shadow-sm">
                            ...
                            <div class="flex min-w-0 gap-3">
                                <div class="avatar placeholder mt-0.5">
                                    <div class="w-10 rounded-full bg-secondary/15 text-secondary">
                                        <span class="text-sm font-bold">{{ $inisialGuru }}</span>
                                    </div>
                                </div>

                                <div class="min-w-0">
                                    <div class="flex flex-wrap items-center gap-2">
                                        <h3 class="font-display text-xl font-semibold leading-snug text-base-content">
                                            {{ $thread->judul }}
                                        </h3>
                            ...
                            <div class="flex shrink-0 gap-2">
                                ...
                                @else
                                    <button
                                        ...
                                        onclick="document.getElementById('lock-thread-{{ $thread->id }}').showModal()"
                    ...
                    <div class="space-y-4 px-5 py-5 sm:px-6">
                        @forelse($thread->pesanDiskusi as $pesan)
                            @php
                                $inisialPesan = strtoupper(substr($pesan->user->name ?? 'U', 0, 1));
                            @endphp
                            ...
                            <div class="flex gap-3 {{ $pesan->is_admin ? 'justify-end' : 'justify-start' }}">
                                ...
                                @if(!$pesan->is_admin)
                                    <div class="avatar placeholder mt-1">
                                        <div class="w-8 rounded-full bg-base-300 text-base-content/70">
                                            <span class="text-xs font-bold">{{ $inisialPesan }}</span>
                                        </div>
                                    </div>
                                @endif
                                ...
                                <div class="max-w-3xl {{ $pesan->is_admin ? 'order-1' : '' }}">
                                    <div class="mb-1 flex items-center gap-2 {{ $pesan->is_admin ? 'justify-end' : '' }}">
                                        <span class="text-xs font-semibold text-base-content/70">
                                            {{ $pesan->is_admin ? 'Anda — Pengawas PAI' : ($pesan->user->name ?? 'Guru') }}
                                        </span>
                                    ...
                                    <div class="rounded-2xl px-4 py-3 text-sm leading-6 shadow-sm {{ $pesan->is_admin ? 'bg-primary text-primary-content rounded-tr-sm' : 'border border-base-300 bg-base-100 text-base-content rounded-tl-sm' }}">
                        ...
                        @empty
                            <div class="rounded-xl border border-dashed border-base-300 bg-base-200 px-4 py-5 text-center text-sm text-base-content/70">
                    ...
                    {{-- Form Balasan --}}
                    @if(!$thread->is_locked)
                        <div class="border-t border-base-300 bg-base-200 px-5 py-4 sm:px-6">
                            <form action="{{ route('admin.diskusi.reply', $thread->id) }}" method="POST">
                                @csrf

                                <label for="reply-{{ $thread->id }}" class="mb-2 block text-sm font-semibold text-base-content">
                                ...
                                <div class="flex flex-col gap-3 sm:flex-row sm:items-end">
                                    <textarea
                                        id="reply-{{ $thread->id }}"
                                        name="isi_pesan"
                                        class="textarea textarea-bordered min-h-24 flex-1 rounded-xl bg-base-100 text-base-content focus:border-primary focus:outline-primary"
                                        rows="3"
                                        placeholder="Tulis arahan atau jawaban untuk guru..."
                                        required
                                    ></textarea>

                                    <button type="submit" class="btn btn-primary rounded-xl sm:min-w-36">
                ...
                {{-- Modal Kunci Thread --}}
                @if(!$thread->is_locked)
                    <dialog id="lock-thread-{{ $thread->id }}" class="modal">
                        <div class="modal-box rounded-2xl">
                            <div class="flex items-start gap-3">
                                <div class="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-warning/15 text-warning">
                                    <span class="material-icons">lock</span>
                                </div>

                                <div>
                                    <h3 class="font-display text-xl font-semibold text-base-content">
                                        Kunci Thread Diskusi?
                                    </h3>

                                    <p class="mt-2 text-sm leading-6 text-base-content/70">
                                        Setelah dikunci, guru tidak dapat mengirim balasan lagi pada thread
                                        <strong>{{ $thread->judul }}</strong>.
                                    </p>
                                </div>
                            </div>
                            ...
                            <div class="modal-action">
                                ...
                                <form action="{{ route('admin.diskusi.lock', $thread->id) }}" method="POST">
                                    @csrf
