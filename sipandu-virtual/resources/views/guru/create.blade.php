...
<div class="max-w-3xl mx-auto">
    <div class="flex items-center gap-3 mb-6">
        <a href="{{ route('admin.guru.index') }}" class="btn btn-circle btn-ghost" aria-label="Kembali ke daftar guru">
            <span class="material-icons">arrow_back</span>
        </a>
        <div>
            <div class="text-sm font-bold tracking-wide uppercase text-secondary">Data Utama</div>
            <h1 class="font-display text-3xl font-semibold">Tambah Guru Binaan</h1>
            <p class="text-neutral/60 mt-1">Isi data guru dengan teliti. NIP/SIAGA tidak boleh sama dengan guru lain.</p>
        </div>
    </div>

    <form action="{{ route('admin.guru.store') }}" method="POST" class="card bg-base-100 border border-base-300 shadow-sm">
        @csrf
        <div class="card-body gap-5">
            <div class="alert alert-info">
                <span class="material-icons">info</span>
                <span>Data ini menjadi acuan saat guru mendaftar dan menunggu persetujuan akun.</span>
            </div>
            ...
            <div class="form-control">
                ...
                <input id="nama_lengkap" name="nama_lengkap" type="text" value="{{ old('nama_lengkap') }}" class="input input-bordered w-full @error('nama_lengkap') input-error @enderror" placeholder="Contoh: Siti Aisyah, S.Pd.I." required autofocus>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div class="form-control">
                    ...
                    <input id="nip_siaga" name="nip_siaga" type="text" value="{{ old('nip_siaga') }}" class="input input-bordered w-full @error('nip_siaga') input-error @enderror" placeholder="Contoh: 199001012020011001">
            ...
            <fieldset class="form-control">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <label class="cursor-pointer border border-base-300 rounded-xl p-4 flex gap-3 items-start hover:bg-base-200">
                        <input type="radio" name="status_jabatan" value="GURU" class="radio radio-primary mt-1" @checked(old('status_jabatan', 'GURU') === 'GURU')>
                        <span><strong class="block">Guru PAI</strong><small class="text-neutral/60">Mengunggah dokumen wajib untuk guru.</small></span>
                    </label>
                    <label class="cursor-pointer border border-base-300 rounded-xl p-4 flex gap-3 items-start hover:bg-base-200">
                        <input type="radio" name="status_jabatan" value="GURU_KEPSEK" class="radio radio-primary mt-1" @checked(old('status_jabatan') === 'GURU_KEPSEK')>
                        <span><strong class="block">Guru PAI + Kepala Sekolah</strong><small class="text-neutral/60">Memiliki tambahan dokumen khusus kepala sekolah.</small></span>
                    </label>
                </div>
            ...
            <label class="flex items-center justify-between p-4 rounded-xl bg-base-200 cursor-pointer">
                <span><strong class="block">Status aktif</strong><small class="text-neutral/60">Guru aktif dapat dicocokkan saat proses registrasi.</small></span>
            ...
            <div class="card-actions justify-end pt-2 border-t border-base-300">
                ...
                <button type="submit" class="btn btn-primary gap-2"><span class="material-icons">save</span> Simpan Guru</button>
            </div>
        </div>
    </form>
</div>
